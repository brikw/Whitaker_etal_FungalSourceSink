...
# drop ING,CAS,KOO site because of low seq depth or incorrect plant sampling
...
median_reads <- median(rowSums(otu_table(ps2))); median_reads 
...
# get abundances
sub <- otu_table(ps3)[,colnames(otu_table(ps3)) %in% rownames(all_sites_ASVs)]@.Data
sort(colSums(sub), dec = TRUE)
...
par(mfrow=c(1,1))
rarecurve(otu_table(ps3), step=20, label = FALSE, col = 'blue') #sample = min.sqn,
par(mar = origmar)

#rarecurve(otu_table(ps3), step=20, label = FALSE, col = 'blue', xlim = c(0,10000))
```
...
```{r, echo=FALSE, results='hide'}
mean <- c(tapply(SbyE$Richness, SbyE$Site, mean))
sd <- c(tapply(SbyE$Richness, SbyE$Site, sd))
count <- c(table(SbyE$Site))
site <- rep(levels(SbyE$Site), 1)
dat <- data.frame("Site"=site, "mean"=mean, "sd"=sd,"reps"=count)
dat$se <- dat$sd/sqrt(dat$reps)
dat$Site <- factor(dat$Site, levels = c(levels(SbyE$Site)))

mean <- c(tapply(SbyE$Richness, SbyE$Alt_Host, mean))
sd <- c(tapply(SbyE$Richness, SbyE$Alt_Host, sd))
count <- c(table(SbyE$Alt_Host))
sample <- rep(levels(SbyE$Alt_Host), 1)
dat2 <- data.frame("Alt_Host"=sample, "mean"=mean, "sd"=sd,"reps"=count)
dat2$se <- dat2$sd/sqrt(dat2$reps)
dat2$Alt_Host <- factor(dat2$Alt_Host, levels = c(levels(SbyE$Alt_Host)))

rich.site <- ggplot(dat, aes(y = mean, x = Site, color = Site)) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.5, size = 1) +
    geom_point(size =3) +
    #geom_boxplot(aes(y = mean, group = Site)) + 
    #geom_jitter(size = 2, position = position_jitter(width = .1)) +
    scale_color_manual(values = rev(site_color)) + guides(color=FALSE) +
    scale_y_continuous("Richness") +
    scale_x_discrete("Site") +
    annotate("text", x=1, y=199, label = "bold((D))", size=4, parse = TRUE) +
    theme(axis.text.x  = element_text(size=9, angle=20, vjust=0.7),
          axis.text.y  = element_text(size=10),
          axis.title.y = element_text(size = 12))
rich.site
rich.host <- ggplot(dat2, aes(y = mean, x = Alt_Host, color = Alt_Host)) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.5, size = 1) +
    geom_point(size =3) +
    ...
    annotate("text", x=.65, y=199, label="bold((C))", size=4, parse = TRUE) +
...
multiplot(rich.host, rich.site, cols=2)
#dev.off()
```

### 2c) Plot Diversity
```{r, echo=FALSE, results='hide'}
#make dataframe(s) for means, st.devs, and st.errors
mean <- c(tapply(SbyE$ShannonDiv, SbyE$Site, mean))
sd <- c(tapply(SbyE$ShannonDiv, SbyE$Site, sd))
count <- c(table(SbyE$Site))
site <- rep(levels(SbyE$Site), 1)
dat3 <- data.frame("Site"=site, "mean"=mean, "sd"=sd,"reps"=count)
dat3$se <- dat3$sd/sqrt(dat3$reps)
dat3$Site <- factor(dat3$Site, levels = c(levels(SbyE$Site)))

mean <- c(tapply(SbyE$ShannonDiv, SbyE$Alt_Host, mean))
sd <- c(tapply(SbyE$ShannonDiv, SbyE$Alt_Host, sd))
count <- c(table(SbyE$Alt_Host))
sample <- rep(levels(SbyE$Alt_Host), 1)
dat4 <- data.frame("Alt_Host"=sample, "mean"=mean, "sd"=sd,"reps"=count)
dat4$se <- dat4$sd/sqrt(dat4$reps)
dat4$Alt_Host <- factor(dat4$Alt_Host, levels = c(levels(SbyE$Alt_Host)))

div.site <- ggplot(dat3, aes(y = mean, x = Site, color = Site)) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.5, size = 1) +
    geom_point(size =3) +
    #geom_boxplot(aes(y = mean, group = Site)) + 
    #geom_jitter(size = 2, position = position_jitter(width = .1)) +
    ...
    scale_x_discrete("Site") +
    annotate("text", x=1, y=3.59, label = "bold((D))", size=4, parse = TRUE) +
    theme(axis.text.x  = element_text(size=9, angle=20, vjust=0.7),
          axis.text.y  = element_text(size=10),
          axis.title.y = element_text(size = 12))
div.site
div.host <- ggplot(dat4, aes(y = mean, x = Alt_Host, color = Alt_Host)) + 
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.5, size = 1) +
    geom_point(size =3) +
    #geom_boxplot(aes(y = ShannonDiv, group = Alt_Host)) + 
    #geom_jitter(size = 2, position = position_jitter(width = .1)) +
    ...
    scale_x_discrete("Sample Type", 
               labels = c("P.hallii", "C4 Grass", "Dicot", "Soil")) + 
    annotate("text", x=.65, y=4, label="bold((C))", size=4, parse = TRUE) +
    theme(axis.text.x  = element_text(size=9, angle=9, vjust=0.7),
          axis.text.y  = element_text(size=10),
          axis.title.y = element_text(size = 12) )
div.host
...
```{r, echo=FALSE, results='hide'}
#tiff("./figures/Diversity by Site and Sample Type.tiff",width=17.4, height=7.6, units="cm", res=600)
...
vd1 <- ggVennDiagram(mySet, label = "count", color = "gray") +
    scale_fill_gradient(low="#EDF8FB",high = "#005824") +
    scale_color_manual(values = c(rep("gray60",3))) +
...
p1 <- ggplot() + coord_equal() + 
    ...
    annotate("text", -40, 22.5, label = "bold((A))", size = 4, parse = TRUE)
...
p2 <- ggplot() + coord_equal() + 
    ...
    theme(axis.text.x  = element_text(size=10),
          axis.text.y  = element_text(size=10) ) +
    #guides(color=FALSE)
    annotate("text", -40, 22.5, label = "bold((B))", size = 4, parse = TRUE)
...
```{r, echo = FALSE, fig.keep = 'none'}
#tiff("./figures/Structure&Richness-by-Sample&Site.tiff",width=17.4, height=17.4, units="cm", res=600)
multiplot(p1+guides(color=FALSE), p2+guides(color=FALSE), rich.host, rich.site, cols = 2)
#dev.off()
```
...
map_st$SourceSink <- as.factor(map_st$SourceSink)
...
#out.st$year <- as.factor(out.st$year)
#out.st$site <- as.factor(out.st$site)
...
means.st <- read.csv("./data/TXEG_SourceTracker_2013_20draws_Local_10RepsMeans.csv", 
...
# then edit the Phallii environments by hand (with Site_Year as the value in the 'Env' column) and rename the file, and read in edited file 
map_grad <- read.csv("./data/TXEG_map_grad_2013_edit.csv", 
                     row.names = 1, stringsAsFactors = TRUE)
...
#grad1 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No1.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad2 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No2.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad3 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No3.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad4 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No4.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad5 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No5.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad6 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No6.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad7 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No7.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad8 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No8.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad9 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No9.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
#grad10 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Gradient_No10.csv", 
#                   row.names = 1, stringsAsFactors = TRUE)
...
#grad.st$year <- as.factor(grad.st$year)
#grad.st$site <- as.factor(grad.st$site)
...
grad.means.st <- read.csv("./data/TXEG_SourceTracker_2013_20draws_Gradient_10RepsMeans.csv",
                          row.names = 1, stringsAsFactors = TRUE)
...
```{r, echo=FALSE, results = 'hide', fig.keep = 'none'}
# read in individual runs
#reg1 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No1.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg2 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No2.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg3 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No3.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg4 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No4.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg5 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No5.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg6 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No6.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg7 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No7.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg8 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No8.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg9 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No9.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
#reg10 <- read.csv("./intermediate/TXEG_SourceTracker_2013_20draws_Regional_No10.csv", 
#                    row.names = 1, stringsAsFactors = TRUE)
...
#reg.st$year <- as.factor(reg.st$year)
#reg.st$site <- as.factor(reg.st$site)
...
reg.means.st <- read.csv("./data/TXEG_SourceTracker_2013_20draws_Regional_10RepsMeans.csv",
                         row.names =1, stringsAsFactors = TRUE)
...
st.p <- 
    ...
    guides(fill=FALSE) +
    annotate("text", 1, 0.0055+max(whole.st$mean[whole.st$sourceType=="C4grass"]),
                            label = "A") + 
...
#    width=23.5/2.54, height=12/2.54)
st.3way
#dev.off()
```
...
###Richness Hyp. tests using Neg.Binom GLM
rich1 <- glm(Richness ~ Alt_Host + Site, data = SbyE, family = negative.binomial(4))
car::Anova(rich1, type = 3)
# function to choose omega/theta value
w <- seq(8, 10, by=0.2)
AIC.values <- rep(0, 5) # allocate vector 
for (i in seq(w))
  AIC.values[i] <- AIC(update(rich1, family=negative.binomial(w[i])))
plot(AIC.values)
min(cbind(w, AIC.values)[,2])  
#want w=theta with lowest AIC
rich2 <- update(rich1, family = negative.binomial(8.2))
car::Anova(rich2, type = 3)
summary(rich2)
```
...
pwt.comm2 <- pairwise(comm.rrpp1, groups = SbyE$Site)
summary(pwt.comm2, confidence = 0.95, test.type = "dist")
```
### 5c) Post Hoc Tests of Diversity
```{r}
...
combo.means <- data.frame(dist.type = as.factor(c(levels(combo.melt$dist.type))),  
                          host.type = as.factor(c(rep(levels(combo.melt$host), each = 4))),
...
blank_data <- data.frame(dist.type = as.factor(c("Sorensen", "Sorensen", 
                  "Euclidean of VST", "Euclidean of VST",
                  "Turnover", "Turnover", "Nestedness", "Nestedness")), 
               host.type = as.factor(c(rep("Phal-C4-between",6), 
                            "Phal-Dicot-between", "Phal-soil-between")),
               mean = c(
                     min(combo.means$mean[combo.means$dist.type=="Sorensen"]), 
                     max(combo.means$mean[combo.means$dist.type=="Sorensen"]) + 0.02,    
                     min(combo.means$mean[combo.means$dist.type=="Euclidean of VST"]), 
                     max(combo.means$mean[combo.means$dist.type=="Euclidean of VST"]) + 1.95,
...
# 7) Plant-Source to Sink Beta Diversity analysis
```{r, echo=FALSE, results = 'hide', fig.height = 4}
### DISTANCE 
# create climate key
climate.key <- SbyE %>%
    dplyr::select(Site, MAP) %>% unique()

# further subset distance data from betapart analysis to data from within site comparisons only
#dim(combo.ls2) # 300 15
phal_plant <- combo.ls2 %>% filter(host == "Phal-Dicot-between" | host == "Phal-C4-between") %>% 
    filter(site == "within")
# add MAP from key
phal_plant$MAP <- climate.key$MAP[match(phal_plant$site.1, climate.key$Site)]
# plot

VST <- ggplot(phal_plant, aes(y = vst.dist, x = MAP, color = host, shape = host)) + 
  geom_point(size = 3) + 
  #ggtitle("P.hallii-Plant Source Dissimilarity \n[Within Sites]") +
  scale_y_continuous("Euclidean of VST") +
  scale_color_manual("Source", labels = c("C4", "Dicot"), 
                     values = c(host_color[c(2:3)])) +
  scale_shape_manual("Source", labels = c("C4", "Dicot"),
                     values = c(16,17))+
  theme(legend.position="bottom")

#tiff("./figures/Distance by Sample Type.tiff",width=8.4, height=9.6, units="cm", res=600)
VST
#dev.off()


phal_dist <- lm(vst.dist ~ host + MAP, data = phal_plant)
#phal_dist2 <- lm(vst.dist ~ host * MAP, data = phal_plant)
#phal_dist3 <- lm(vst.dist ~ host * MAP + I(MAP^2), data = phal_plant)
#phal_dist4 <- lm(vst.dist ~ host + MAP + I(MAP^2), data = phal_plant)

#anova(phal_dist, phal_dist2)
#anova(phal_dist2, phal_dist3)
#anova(phal_dist, phal_dist4); anova(phal_dist3, phal_dist4)

Anova(phal_dist, type = "III")
summary(phal_dist)
#Anova(phal_dist2, type = "III")
#Anova(phal_dist3, type = "III")
#Anova(phal_dist4, type = "III")
```
