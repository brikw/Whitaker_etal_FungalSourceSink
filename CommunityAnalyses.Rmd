...
map.st.p <- 
    ggplot(whole.st, aes(x = MAP, y = mean, 
                        group = sourceType, color = sourceType, shape = scale)) +
    #facet_grid(. ~ scale) +
...
combo.melt <- melt(combo.ls2, measure.vars = c("Turnover", "Nestedness",
                                 "Sorensen", "vst.dist"),
                    variable.name = "dist.type", value.name = "betaDiv")
levels(combo.melt$dist.type)[4] <- "Euclidean of VST"
...
combo.means$dist.type <- factor(combo.means$dist.type, 
    levels = c("Turnover", "Nestedness", "Sorensen", "Euclidean of VST"))
```

```{r, echo=FALSE, results = 'hide', fig.height = 7}
beta.names <- c("Turnover" = ' A)  Turnover',
                "Nestedness" = ' B)  Nestedness',
                "Sorensen" = ' C) Sorensen',
                "Euclidean of VST" = ' D) Euclidean of VST')
...
blank_data <- data.frame(dist.type = c("Turnover", "Turnover", "Nestedness", 
        "Nestedness", "Sorensen", "Sorensen", "Euclidean of VST", "Euclidean of VST"), 
               host.type = c(rep("Phal-C4-between",6), #"Phal-Monocot-between",
                            "Phal-Dicot-between", "Phal-soil-between"),
               mean = c(min(combo.means$mean[combo.means$dist.type=="Turnover"]), 
